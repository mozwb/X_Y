...
```
## 补充
    由于之前考虑用宏实现跨平台，但是经过考虑貌似不太好，所以使用了工厂模式，但是分层设计是差不多的，依旧是XWidget供使用，然后base层消除平台差异，已经没有core层了，用虚函数写了一个WindowImpl来实现对各个操作系统的转发，这样子的话其实base层就已经是封装完好的了，但是为了适配之前写的代码，同时为了避免暴露底层方便使用，所以感觉转发一下貌似会更整洁
</details>